...
app.post('/servers', async (req, res) => {
  ...
  const server = data[0];
  // Auto-join the owner and create a #general channel immediately
...
app.post('/join-server', async (req, res) => {
  ...
  if (error) return res.status(400).json({ error: "Already joined or invalid ID" });
...
app.get('/messages/:id', async (req, res) => {
  ...
  if (isDM) {
    query = query.eq('dm_room_id', req.params.id);
  } else {
    query = query.eq('channel_id', req.params.id);
  }
